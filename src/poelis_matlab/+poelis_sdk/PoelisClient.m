classdef PoelisClient < handle
    ...
    %   value = client.get_value('workspace.product.item.property');  % Returns MATLAB double/string
    %
    % Properties:
    ...
    methods
        ...
        function value = get_value(obj, path)
            ...
            %
            % Example:
            %   value = client.get_value('demo_workspace.demo_product.demo_item.demo_property_mass');
            ...
            try
                ...
                elseif contains(ME.message, 'must include an item')
                    error('poelis:PropertyPathRequiresItem', ...
                        ['Property paths must include an item before the property name.\n' ...
                         'Read example: workspace.product.item.property\n' ...
                         'Read example with baseline: workspace.product.baseline.item.property\n' ...
                         'Path: %s\n' ...
                         'Original error: %s'], ...
                        path, ME.message);
        ...
        function change_property(obj, path, value, title, description)
            ...
            try
                ...
                elseif contains(ME.message, 'must include an item')
                    error('poelis:PropertyPathRequiresItem', ...
                        ['Property paths must include an item before the property name.\n' ...
                         'Write example: workspace.product.draft.item.property\n' ...
                         'Path: %s\n' ...
                         'Original error: %s'], ...
                        path, ME.message);
                elseif contains(ME.message, 'forbidden') || contains(ME.message, 'permission denied') || ...
